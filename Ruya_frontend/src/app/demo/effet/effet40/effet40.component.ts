...
export class Effet40Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscription = this.ajouterFichierService.fichiers$.subscribe(data => {
      console.log('🔍 DEBUG Effet40 - Tous les fichiers reçus:', data);
      ...
      this.fichiers = (data || []).filter(f => {
        ...
        // Accepte UNIQUEMENT le code 40
        const isValidCode = code === '40';
        ...
        if (match) {
          console.log('✅ Fichier correspond aux critères effet40');
        }
        
        return match;
      });
      
      console.log('📊 Fichiers filtrés pour effet40:', this.fichiers);
      this.applyFilters();
