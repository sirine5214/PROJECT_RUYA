...
export class Prelevement20Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscription = this.ajouterFichierService.fichiers$.subscribe(data => {
      console.log('🔍 DEBUG Prlv20 - Tous les fichiers reçus:', data);
      ...
      this.fichiers = (data || []).filter(f => {
        ...
        // Accepte UNIQUEMENT le code 20
        const isValidCode = code === '20';
        ...
        if (match) {
          console.log('✅ Fichier correspond aux critères prlv20');
        }
        
        return match;
      });
      
      console.log('📊 Fichiers filtrés pour prlv20:', this.fichiers);
      this.applyFilters();
