...
export class Cheque31Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscription = this.ajouterFichierService.fichiers$.subscribe(data => {
      console.log('🔍 DEBUG Cheque31 - Tous les fichiers reçus:', data);
      ...
      this.fichiers = (data || []).filter(f => {
        ...
        // Accepte UNIQUEMENT le code 31
        const isValidCode = code === '31';
        ...
        if (match) {
          console.log('✅ Fichier correspond aux critères cheque31');
        }
        
        return match;
      });
      
      console.log('📊 Fichiers filtrés pour cheque31:', this.fichiers);
      this.applyFilters();
