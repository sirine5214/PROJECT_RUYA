...
export class Cheque32Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscription = this.ajouterFichierService.fichiers$.subscribe(data => {
      console.log('🔍 DEBUG Cheque32 - Tous les fichiers reçus:', data);
      ...
      this.fichiers = (data || []).filter(f => {
        ...
        // Accepte UNIQUEMENT le code 32
        const isValidCode = code === '32';
        ...
        if (match) {
          console.log('✅ Fichier correspond aux critères cheque32');
        }
        
        return match;
      });
      
      console.log('📊 Fichiers filtrés pour cheque32:', this.fichiers);
      this.applyFilters();
