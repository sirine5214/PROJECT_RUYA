...
export class Cheque30Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ajouterFichierService.fichiers$.subscribe(data => {
      console.log('🔍 DEBUG Cheque30 - Tous les fichiers reçus:', data);
      ...
      this.fichiers = (data || []).filter(f => {
        ...
        if (match) {
          console.log('✅ Fichier correspond aux critères cheque30');
        }
        
        return match;
      });
      
      console.log('📊 Fichiers filtrés pour cheque30:', this.fichiers);
      this.applyFilters();
