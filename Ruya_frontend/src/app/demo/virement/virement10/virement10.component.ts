...
export class Virement10Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscription = this.ajouterFichierService.fichiers$.subscribe(data => {
      console.log('🔍 DEBUG Virement10 - Tous les fichiers reçus:', data);
      ...
      this.fichiers = (data || []).filter(f => {
        ...
        // Accepte UNIQUEMENT le code 10
        const isValidCode = code === '10';
        ...
        if (match) {
          console.log('✅ Fichier correspond aux critères virement10');
        }
        
        return match;
      });
      
      console.log('📊 Fichiers filtrés pour virement10:', this.fichiers);
      this.applyFilters();
